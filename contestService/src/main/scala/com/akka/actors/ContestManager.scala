...
import com.akka.actors.ContestActor.{Active, Completed, ContestResponse}
import com.akka.actors.DatabaseManager.DBContestCreated
import com.akka.persistance.ContestDaoImpl
import com.akka.persistance.Model.Contest
...
object ContestManager {
  ...
  def apply(currentContests: Map[UUID, Contest] = Map.empty,
    ...
    Behaviors.setup { ctx =>
      ...
      val supervisedBehavior = Behaviors
      ...
      val dbManagerPool = Routers.pool(4)(supervisedBehavior).withRouteeProps(dispatcher).withRoundRobinRouting()
      ...
      .receive[Command] { (ctx, msg) =>
        ...
        msg match {
          ...
          case CreateContest(contestId, name, durationInMinutes) =>
            ...
            val dbResultActorRef: ActorRef[DatabaseManager.Result] = ctx.messageAdapter(r => WrappedDatabaseResult(contestId, r))
