...
import com.akka.actors.ContestManager.GetContestResponse
import com.akka.persistance.Model._
...
class ContestRoutes(contest: ActorRef[ContestManager.Command])(implicit val system: ActorSystem[_]) {
      ...
      pathPrefix("contest") {
        concat(
          ...
          pathPrefix(JavaUUID) { id =>
            concat(
              pathEndOrSingleSlash {
                concat(get(complete(getContest(id))))
              },
              (get & path("start")) {
  ...
  def getContest(id: UUID): Future[ApiResponse] = {
    ...
    val contestInfoFuture: Future[ContestManager.Response] = contest.ask(ContestManager.GetContest(id, _: ActorRef[ContestManager.Response]))
